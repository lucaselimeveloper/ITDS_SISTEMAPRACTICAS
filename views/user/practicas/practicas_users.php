...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['generar_reporte'])) {
    ...
    if (isset($_POST['search_value']) && !empty($_POST['search_value'])) {
        ...
        $sql .= " WHERE (e.codigo LIKE '%$search%' OR e.ci LIKE '%$search%' OR e.nombre_completo LIKE '%$search%' 
    ...
    if (isset($_POST['filtro_estado']) && !empty($_POST['filtro_estado'])) {
        $filtro = mysqli_real_escape_string($conexion, $_POST['filtro_estado']);
        ...
        if ($filtro === 'en_progreso') {
            $sql .= " pp.estado = 'activa' AND pp.fecha_fin IS NULL";
        } elseif ($filtro === 'finalizada') {
            $sql .= " pp.estado = 'finalizada'";
        } elseif ($filtro === 'congelada') {
            $sql .= " pp.estado = 'congelada'";
        }
    }
...
<body class="bg-gray-100">
    ...
    <script>
        ...
        let currentFiltroEstado = '<?= $filtro_estado ?>';

        function aplicarFiltro() {
            const filtro = document.getElementById('filtroEstado').value;
            currentFiltroEstado = filtro;
            window.location.href = `practicas_user.php?estado=${filtro}`;
        }
        ...
        function printModal() {
            ...
            // Esperar un momento para que cargue el contenido antes de imprimir
            setTimeout(() => {
                printWindow.print();
            }, 500);
        }
        ...
        document.getElementById('reportButton').addEventListener('click', function() {
            ...
            filtroInput.name = 'filtro_estado';
            filtroInput.value = currentFiltroEstado;
